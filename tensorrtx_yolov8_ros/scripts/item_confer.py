...
class PoseAdjuster:
    ...
    def __init__(self):
        ...
        self.adjustment_directions = []  # 调整方向序列
        
    def generate_adjustment_sequence(self):
        """生成调整序列：先正转45度，再逆转45度"""
        self.adjustment_directions = []
        
        # 正方向调整（0° → +45°）
        positive_steps = int(self.max_pitch_adjustment / self.pitch_step)
        for i in range(1, positive_steps + 1):
            self.adjustment_directions.append(i * self.pitch_step)
        
        # 负方向调整（0° → -45°）
        for i in range(1, positive_steps + 1):
            self.adjustment_directions.append(-i * self.pitch_step)
        
        return self.adjustment_directions
    ...
    def get_adjustment_value(self, attempt_count):
        ...
        if attempt_count - 1 < len(self.adjustment_directions):
            return self.adjustment_directions[attempt_count - 1]
        
        return None  # 超出调整范围
    
    def adjust_pose_pitch(self, pose, pitch_adjustment):
        """调整姿态的俯仰角"""
        # 获取原始四元数
        orig_quat = [
        ...
        # 调整俯仰角（pitch）
        new_euler = [orig_euler[0], orig_euler[1] + pitch_adjustment, orig_euler[2]]
...
class PoseTransformer:
    def __init__(self):
        ...
        self.max_adjustment_attempts = 36  # 18个正方向 + 18个负方向 = 36次尝试
    ...
    def publish_adjusted_pose(self, attempt_count):
        """发布调整后的姿态"""
        adjustment_value = self.pose_adjuster.get_adjustment_value(attempt_count)
        ...
        # 调整姿态
        adjusted_pose = self.pose_adjuster.adjust_pose_pitch(
            self.original_target_pose, adjustment_value
        )
        ...
        if attempt_count == 0:
            rospy.loginfo(f"Attempt {attempt_count}: Original pose, pitch = {np.degrees(euler[1]):.1f}°")
        else:
            rospy.loginfo(f"Attempt {attempt_count}: Adjusted pitch = {np.degrees(adjustment_value):.1f}°, "
    ...
    def check_ik_status(self):
        ...
        if self.ik_manager.ik_status_received:
            if self.ik_manager.ik_success:
                ...
                # 逆解失败，等待1秒后尝试下一个调整
                if current_time - self.ik_check_start_time > 0.2:
